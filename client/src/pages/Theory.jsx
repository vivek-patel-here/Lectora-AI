...
function Theory({openModelFunction}) {
  ...
  return (
    <div className="min-h-screen  w-screen flex flex-col relative items-center">
      ...
      <div className="min-h-[90vh] gap-5 w-full flex flex-col  justify-between p-4 ">
          ...
          <div className="w-full h-fit text-lg lg:w-9/10 text-gray-950 bg-gray-200 p-3 rounded-md">
            ...
            <div className=" min-h-60 w-full flex flex-wrap items-center gap-5 px-2 py-1">
              {youtubeIds.map((video_id,it) => {
                return (
                  <div key={it} className=" border relative  border-[#40404050] h-fit w-fit  md:h-60 md:w-90 md:p-4 rounded-xl hover:shadow-2xl hover:scale-105 transition-all" onClick={()=>{openModel()}}>
                    <a href="#ytmodel"><div className="absolute h-50  w-80 z-10 text-white bg-transparent" onClick={()=>openModelFunction(video_id)}></div></a>
                    <iframe
                      width="300"
                      height="200"
        ...
        {quizzes.length > 0 && (
          <div className="w-full h-fit text-lg lg:w-9/10 text-gray-950 bg-gray-200 p-3 rounded-md">
            ...
            <div className=" min-h-60 w-19/20 flex flex-col gap-2 px-2 py-1">
              {quizzes.map((quiz,it) => {
                return (
                <Quizz key={it} quizz={quiz} it={it} isSubmitted={isSubmitted}/>
                );
              })}
               {!isSubmitted && <button className="h-fit w-fit py-1 px-3 font-bold  rounded-xl border" onClick={()=>setIsSubmitted(true)}> Submit</button>}
           </div>
          </div>
        )}
